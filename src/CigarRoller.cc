...
{
  CigarRoller c1_roller, c2_roller;
  c1_roller.Set(c1.c_str());
  c2_roller.Set(c2.c_str());
  string expanded = c1_roller.getExpandedString()+c2_roller.getExpandedString();
  return condense_repeat_string(expanded);
}