...
double cal_single_base_depth_oc(string chromosome, uint64_t pos, samfile_t *fp, bam_index_t *idx_bam)
{
  const int minMaqQ = 1;
  /// Parse region
  int tid_t;
  ...
  /// Create bam iterator
  bam_iter_t iter_bam;
  /// get the alignments that left to 2 * read length to the left_pos and right in the right_pos
  /// due to deletions, the fetching position should start from 2 read length to the left
  /// left_pos and right_pos is 0-based and bam_iter_query takes 0-based positions as parameters
...
double cal_mean_depth_oc(std::string chrom, uint64_t start, uint64_t end, samfile_t *fp, bam_index_t *idx_bam)
{
  ...
  {
    condition = (b->core.qual >= 0) && (!(b->core.flag & BAM_FDUP)) && (b->core.flag & BAM_FPAIRED);
//    set_intersection(start,end,alignment.getAlignmentStart(),alignment.getAlignmentEnd(),);
    if (condition)
    {
      alignment.readBamRecord(b);
      CigarRoller tmp_cigar;
      string oc_tag;
      oc_tag = alignment.originCigar();
